...
Deno.serve(async (req) => {
  ...
  const { agreementId, channel, sendTo, reminder } = await req.json();
  ...
  try {
    providerMessageId = channel === "sms"
      ? await sendSms(sendTo, prefill, link, reminder)
      : await sendEmail(sendTo, prefill, link, reminder);
  } catch (err) {
  ...
  // First send flips draft -> sent. A reminder must not downgrade "viewed".
  ...
  return json({ ok: true, channel, sentTo: sendTo, reminder: !!reminder });
});
...
async function sendSms(
  ...
  const body = reminder
    ? `Tailgate Co: just checking in on the discount partnership agreement for ` +
      `${prefill.business_name} — sign here when you get a sec: ${link}\n` +
...
async function sendEmail(
  ...
  const biz = escapeHtml(prefill.business_name ?? "your business");
  const subject = reminder
    ? "Just checking in on your Tailgate agreement"
    : "Your Tailgate discount partnership agreement";
  const heading = reminder
    ? "Just checking in"
    : "Your agreement is ready to sign";
  const intro = reminder
    ? `Following up to see if you've had a chance to look at the discount partnership agreement for ${biz}. It only takes a minute — read it, confirm your discount, and sign.`
    : `Everything is filled in for ${biz}. Give it a read, confirm your discount, and add your signature.`;
