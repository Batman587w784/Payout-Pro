...
//
// Required secrets (Supabase Dashboard → Edge Functions → Secrets, or `supabase secrets set`):
//   RESEND_API_KEY  — your Resend API key
//   FROM_EMAIL      — verified sender, e.g.  Tailgate Fundraising <info@yourdomain.com>
//
// Called from the app via supabase.functions.invoke('send-info-email', { body: {...} }),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const payload: Record<string, unknown> = { from: FROM_EMAIL, to: [to], subject, text };
    ...
    if (!res.ok) return json({ error: data?.message || "Resend rejected the email." }, 400);
